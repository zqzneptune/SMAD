#include <Rcpp.h>
#ifndef INT_FASTMAT_HPP_
# define INT_FASTMAT_HPP_
#include <valarray>
...
public:
	Fastmat<T>& operator=(const Fastmat<T>& other){
		if(other.n_rows == n_rows && other.n_cols == n_cols)
			mat = other.mat;
		else throw std::runtime_error("assignment operator error: dimensions mismatch");
		return *this;
	}
	...
	std::slice_array<T> operator()(const int &x, const Span &) {
		return mat[std::slice(x * n_cols, n_cols, 1)];
	}
	std::valarray<T> operator()(const int &x, const Span &) const {
		return mat[std::slice(x * n_cols, n_cols, 1)];
	}
	std::slice_array<T> operator()(const Span &, const int &y) {
		return mat[std::slice(y, n_rows, n_cols)];
	}
	std::valarray<T> operator()(const Span &, const int &y) const {
		return mat[std::slice(y, n_rows, n_cols)];
	}
	...
	std::ostream& print(std::ostream& out = Rcpp::Rcout) const {
...
typedef Fastmat<Quant_t> Quantmat;

template<typename T>
std::ostream& operator<<(std::ostream& out, Fastmat<T> const& mat) {
	return mat.print(out);
}

template<typename T>
struct vector1 : std::vector<T> {
private:
	T zero_element;
public:
...
template<typename T>
struct valarray1 : std::valarray<T> {
	valarray1(const std::valarray<T>& other) : std::valarray<T>(other){}
	double mean() const {
		return this->sum()/static_cast<double>(this->size());
	}
	double var() const {
		double mu = mean();
		std::valarray<T> tmp(*this - mu);
...
template<>
struct valarray1<bool> : std::valarray<bool> {
	double mean() const {
